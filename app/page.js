...
const pdfTbl = (headers, rows, foot) => `<table style="width:100%;border-collapse:collapse;border:1px solid #e8e6e0;border-radius:10px;overflow:hidden;margin-bottom:28px;"><thead><tr>${pdfTh(headers)}</tr></thead><tbody>${rows}${foot||''}</tbody></table>`
const pdfStat = (label, value, color) => `<div style="background:#f9f9fb;border-radius:10px;padding:14px 16px;border:1px solid #e8e6e0;"><div style="font-size:10px;text-transform:uppercase;letter-spacing:0.06em;color:#aaa;margin-bottom:6px;">${label}</div><div style="font-size:20px;font-weight:800;color:${color||'#111118'};">${value}</div></div>`
...
export default function App() {
  ...
  const save = async () => {
    if (modal === 'addClient') {
      ...
      const d = { amount: Number(form.amount)||0, date: form.date, status: form.status, notes: form.notes||'', invoice_number: form.invoice_number||null, invoice_url: form.invoice_url||null }
      ...
      const d = { sponsor_name: form.sponsor_name||'', amount: Number(form.amount)||0, date: form.date, status: form.status, joy_contribution: !!form.joy_contribution, notes: form.notes||'', invoice_number: form.invoice_number||null, payment_url: form.payment_url||null }
  ...
  const exportEventPDF = (event) => {
    ...
    const spent = evtSpent(event.id), gross = evtGross(event.id), jc = evtJoy(event.id)
    ...
    ${tbl(['Date','Amount','Status','Invoice #','Notes'],invs.map(i=>`<tr>${td([i.date,f(i.amount),i.status,i.invoice_number||'—',i.notes||''])}</tr>`).join(''))}
    ${spons.length>0?`<h3 style="font-size:14px;font-weight:700;margin:0 0 10px;">Sponsor payments</h3>${tbl(['Date','Sponsor','Amount','Status','Type','Invoice #','Notes'],spons.map(s=>`<tr>${td([s.date,s.sponsor_name,f(s.amount),s.status,s.joy_contribution?'Joy contribution':'External',s.invoice_number||'—',s.notes||''])}</tr>`).join(''))}` :''}
  ...
  const exportClientPDF = (client) => {
    ...
    const eventSection = ev => {
      ...
      <div style="margin-bottom:32px;padding-top:24px;border-top:2px solid #e8e6e0;">
        ...
        ${evInvs.length>0?`${subhead('Invoices')}${tbl(['Date','Amount','Status','Invoice #','Notes'],evInvs.map(i=>`<tr>${td([i.date,f(i.amount),i.status,i.invoice_number||'—',i.notes||''])}</tr>`).join(''))}`:''}
        ${evSpons.length>0?`${subhead('Sponsor payments')}${tbl(['Date','Sponsor','Amount','Status','Type','Invoice #','Notes'],evSpons.map(s=>`<tr>${td([s.date,s.sponsor_name,f(s.amount),s.status,s.joy_contribution?'Joy contribution':'External',s.invoice_number||'—',s.notes||''])}</tr>`).join(''))}`:''}
      </div>`
    }
    ...
    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:32px;padding-bottom:20px;border-bottom:3px solid #330066;">
      <div>
        <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.1em;color:#330066;font-weight:700;margin-bottom:8px;">Joy Life — Client Summary</div>
        <h1 style="font-size:26px;font-weight:900;letter-spacing:-0.03em;margin-bottom:8px;">${client.name}</h1>
        <div style="font-size:13px;color:#888;">${client.service_rate}% commission${client.contact?' &nbsp;·&nbsp; '+client.contact:''}</div>
      </div>
      <button class="no-print" onclick="window.print()" style="font-size:13px;padding:10px 20px;background:#330066;color:white;border:none;border-radius:10px;cursor:pointer;font-weight:700;">Print / Save PDF</button>
    </div>
  ...
  const renderEventDetail = event => {
    ...
    return (
      <div style={wrap}>
        ...
        <div style={{ marginBottom: 32 }}>
          <DataTable
            headers={['Date', 'Amount', 'Status', 'Invoice #', 'Notes', 'Link', 'Modified', '']}
            rows={invs.map(i => (
              <tr key={i.id} onMouseEnter={ev => ev.currentTarget.style.background = C.offWhite} onMouseLeave={ev => ev.currentTarget.style.background = ''}>
                ...
                <td style={{ padding: '8px 18px', borderBottom: `1px solid ${C.borderLight}`, whiteSpace: 'nowrap' }}>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <Btn size="sm" onClick={() => openModal('editInvoice', { id: i.id, amount: i.amount, date: i.date, status: i.status, notes: i.notes, invoice_number: i.invoice_number, invoice_url: i.invoice_url })}>Edit</Btn>
        ...
        {(gcio || spons.length > 0) && (
          <>
            ...
            <DataTable
              headers={['Date', 'Sponsor', 'Amount', 'Status', 'Type', 'Invoice #', 'Notes', 'Link', '']}
              rows={spons.map(sp => (
                <tr key={sp.id} onMouseEnter={ev => ev.currentTarget.style.background = C.offWhite} onMouseLeave={ev => ev.currentTarget.style.background = ''}>
                  ...
                  <td style={{ padding: '8px 18px', borderBottom: `1px solid ${C.borderLight}`, whiteSpace: 'nowrap' }}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <Btn size="sm" onClick={() => openModal('editSponsor', { id: sp.id, sponsor_name: sp.sponsor_name, amount: sp.amount, date: sp.date, status: sp.status, joy_contribution: sp.joy_contribution, notes: sp.notes, invoice_number: sp.invoice_number, payment_url: sp.payment_url })}>Edit</Btn>
  ...
  const renderAllInvoices = () => (
    <div style={wrap}>
      ...
      <DataTable
        headers={['Date', 'Client', 'Event', 'Amount', 'Status', 'Invoice #', 'Notes', 'Link', 'Modified', '']}
  ...
  const renderAllSponsors = () => {
    ...
    return (
      <div style={wrap}>
        ...
        <DataTable
          headers={['Date', 'Event', 'Sponsor', 'Amount', 'Status', 'Type', 'Invoice #', 'Notes', 'Link', '']}
      ...
      <div style={{ position:'fixed', inset:0, background:'rgba(17,17,24,0.45)', backdropFilter:'blur(6px)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:200 }}
        ...
        <div style={{ background:C.white, borderRadius:22, padding:32, width:460, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 32px 80px rgba(0,0,0,0.22)' }}>
          ...
          {(modal==='addInvoice'||modal==='editInvoice') && <>
            ...
            <Field label="Invoice number (optional)"><input style={inputSt} value={form.invoice_number||''} onChange={e=>sf('invoice_number',e.target.value)} placeholder="e.g. INV-1042" /></Field>
  ...
  return (
    <div style={{ fontFamily:'-apple-system,BlinkMacSystemFont,"Inter","Segoe UI",sans-serif', fontSize:14, color:C.ink, background:C.offWhite, minHeight:'100vh' }}>
      {/* Topbar + tabs combined */}
      <div style={{ background:C.white, borderBottom:`1px solid ${C.border}`, padding:'0 32px', position:'sticky', top:0, zIndex:50, boxShadow:'0 1px 8px rgba(0,0,0,0.04)' }}>
        <div style={{ maxWidth:1200, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', height:54 }}>
          <div style={{ display:'flex', alignItems:'center', gap:32 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <JoyLogoDark size={22} />
              <span style={{ color:C.inkFaint, fontSize:13, fontWeight:500 }}>Event Tracker</span>
            </div>
            <div style={{ display:'flex', gap:2 }}>
              {tabs.map((t,i) => (
                <button key={t} onClick={() => navTo(t)} style={{
                  ...
                  background: page===t ? C.purpleLight : 'none',
